...
class FunctionSignatureExtractor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        self.function_signatures[node.name] = f"({', '.join(arg_names)})"
...
class ClassExtractor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        if self.current_class:
            ...
            self.classes[self.current_class]["methods"][f"{self.current_class}::{node.name}"] = f"({', '.join(arg_names)})"

    def visit_Assign(self, node):
        if self.current_class:
            for target in node.targets:
                if isinstance(target, ast.Attribute) and isinstance(target.value, ast.Name) and target.value.id == 'self':
                    self.classes[self.current_class]["fields"][f"{self.current_class}::self::{target.attr}"] = None
    ...
    def _process_classes_data(classes):
        ...
        for class_name, class_data in classes.items():
            ...
            for method_name, signature in class_data["methods"].items():
                classes_data[class_name]["methods"].append(f"{method_name}{signature}")
